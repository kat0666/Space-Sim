...
const Controls: React.FC<ControlsProps> = ({
  ...
  return (
    <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-gray-900/80 backdrop-blur-md border border-gray-700 p-4 rounded-xl flex items-center gap-6 shadow-2xl z-20">
      {/* Time Controls */}
      ...
      {/* Camera Controls */}
      <div className="flex flex-col items-center border-r border-gray-600 pr-4">
        <label className="text-xs text-gray-400 mb-1">Camera Rotation</label>
        <div className="flex gap-2 items-center">
            <button 
                className="bg-gray-800 hover:bg-gray-700 text-white px-2 py-1 rounded text-lg"
                onClick={() => setCamera(c => ({...c, rotation: c.rotation - 0.1}))}
            >↺</button>
            <span className="text-xs w-10 text-center">{(camera.rotation * 180 / Math.PI).toFixed(0)}°</span>
            <button 
                className="bg-gray-800 hover:bg-gray-700 text-white px-2 py-1 rounded text-lg"
                onClick={() => setCamera(c => ({...c, rotation: c.rotation + 0.1}))}
            >↻</button>
             <button 
                className="bg-gray-800 hover:bg-gray-700 text-white px-2 py-1 rounded text-xs ml-2"
                onClick={() => setCamera(c => ({...c, x:0, y:0, zoom: 0.5, rotation: 0}))}
            >RESET</button>
        </div>
      </div>
